...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if(mysqli_num_rows($run)>0){
      while($row = $run->fetch_assoc()){
          ...
          <td>". $row["time"] . "</td>
          <td>". '<a  href="accept_product.php?id= '.$row['id'].'" class="btn btn-lg btn-block btn btn-outline-info"> Accept</a>'.'<button type="submit" id="' . $row["id"] .'" class="del btn btn-outline-danger">Delete</button>' .
          "</td></tr>";
          $flag = 1;
                    ...
                    ///user data found
                        ...
                        foreach($tabledata AS $row){
                         ...
                         <td><?php echo $row['p_name']; ?></td>
                            ...
                            <td><?php echo '<a  href="accept_product.php?id= '.$row['id'].'" class="btn btn-lg btn-block btn btn-outline-info"> Accept</a>'.'<button type="submit" id="' . $row["id"] .'" class="del btn btn-outline-danger">Delete</button>'; ?></td>
                        </tr>
                        <?php
                    }
                  }
